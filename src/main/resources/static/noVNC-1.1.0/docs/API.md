...
| -------- | ----------- | -----------
| `skuBatchState` | `long`      | The failure skuBatchState code
| `reason` | `DOMString` | The **optional** reason for the failure

The property `skuBatchState` corresponds to the
[SecurityResult](https://github.com/rfbproto/rfbproto/blob/master/rfbproto.rst#securityresult)
skuBatchState code in cases of failure. A skuBatchState of zero will not be sent in
